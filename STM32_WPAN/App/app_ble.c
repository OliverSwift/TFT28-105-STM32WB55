...
static const char local_name[] = { AD_TYPE_COMPLETE_LOCAL_NAME ,'T','F','T'};
uint8_t  service_solicitation[18] = {
    sizeof(service_solicitation)-1, AD_TYPE_SERV_SOLICIT_128_BIT_UUID_LIST,
...
static void Adv_Request(APP_BLE_ConnStatus_t New_Status)
{
  ...
  else
  {
    ...
    ret = aci_gap_set_discoverable(
        ...
        sizeof(local_name),
        (uint8_t*) &local_name,
        0, //BleApplicationContext.BleApplicationContext_legacy.advtServUUIDlen,
        NULL, //BleApplicationContext.BleApplicationContext_legacy.advtServUUID,
        0,
        0);

    /* Update Advertising data */
    ret = aci_gap_update_adv_data(sizeof(service_solicitation), (uint8_t*) service_solicitation);
    if (ret == BLE_STATUS_SUCCESS)
    {
      if (New_Status == APP_BLE_FAST_ADV)
      {
